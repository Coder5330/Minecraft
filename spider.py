...
class Spider(Mob):
    ...
    def update(self):
        ...
        if hostile:
            if dist_3d > SPIDER_ATTACK_RANGE:
                self.move((dx/dist) * SPIDER_SPEED * dt,
                          (dz/dist) * SPIDER_SPEED * dt)
            else:
            ...
            if random.random() < 0.2:
                self.move(math.cos(self.yaw) * SPIDER_SPEED * 0.3 * dt,
                          math.sin(self.yaw) * SPIDER_SPEED * 0.3 * dt)
    ...
    def render(self):
        ...
        try:
            ...
            pass        if not is_night and self._aggro:
            self._aggro = False
