...
class MobManager:
    ...
    def update(self):
        ...
        if is_night and self._hostile_count() < MAX_HOSTILE and random.random() < 0.02:
    ...
    def handle_player_attack(self):
        ...
        for mob in self.mobs:
            if not mob.alive:
                continue
            # aim at the mob's centre (0.5 blocks above its feet)
            mob_centre = glm.vec3(mob.position.x, mob.position.y + 0.5, mob.position.z)
