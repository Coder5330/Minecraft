...
SHEEP_SPEED = 0.00035
WANDER_CHANGE_MS = 3500

class Sheep(Mob):
    """Passive mob — wanders randomly, flees when hit, drops mutton."""
    def __init__(self, app, position):
        super().__init__(app, position, health=8)
    ...
    def _load_texture(self):
        img = pg.image.load('assets/sheep.png').convert_alpha()
        w, h = img.get_size()
    ...
    def _build_vao(self):
        ...
        body_hw, body_hh, body_hl = 0.48, 0.38, 0.62
        body_cy = 0.65
        leg_w, leg_h = 0.15, 0.52
    ...
    def take_damage(self, amount):
        ...
        if was_alive and not self.alive:
            self.app.scene.dropped_items.spawn(ITEM_MUTTON, self.position + glm.vec3(0, 0.5, 0), 1)
        else:
            self._fleeing = True
